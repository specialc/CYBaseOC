...
+ (void)event:(UIEvent *)event {
    ...
    dispatch_once(&onceToken, ^{
        queue = dispatch_queue_create("cy.save_test_img", DISPATCH_QUEUE_SERIAL);
    });
...
+ (void)requestEvent:(UIImage *)image heartbeat:(BOOL)heartbeat {
    ...
    NSData *scale_img_data = UIImageJPEGRepresentation(scale_img, 0.4);
