...
@implementation NSDate (CY_Category)
...
// 时间详情
- (struct CY_DateDetail)dateDetail {
    // 年月日
...
+ (void)getHour:(NSInteger *)hour minute:(NSInteger *)minute second:(NSInteger *)second fromSurplusTime:(NSTimeInterval)surplusTime {
    ...
    *hour = surSecond / aHour;
    ...
    if (surSecond != 0) {
        *minute = surSecond / aMinute;
    }
    
    surSecond = surSecond % aMinute;
    if (surSecond != 0) {
        *second = surSecond;
    }
